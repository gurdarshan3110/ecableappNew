...
function subLedger($id){
    ...
    //echo "SELECT `MONTH_DATE`,AMOUNT_TYPE,AMOUNT,RECEIPT_NO,DISCOUNT,SPECIAL_REMARKS,`ID` FROM `monthly_charges` WHERE SUBSCRIBER_ID='$id' AND STATUS='A' GROUP BY `ID` ORDER BY `ID` ASC";
    $collection=$objMisc->getAllrecordsNew("SELECT `MONTH_DATE`,AMOUNT_TYPE,AMOUNT,RECEIPT_NO,DISCOUNT,SPECIAL_REMARKS,`ID` FROM (SELECT `MONTH_DATE`,AMOUNT_TYPE,AMOUNT,RECEIPT_NO,DISCOUNT,SPECIAL_REMARKS,`ID` FROM `monthly_charges` WHERE SUBSCRIBER_ID='$id' AND STATUS='A' AND RECEIPT_NO!='Opening Balance' GROUP BY `ID` ORDER BY `ID` DESC LIMIT 20) t  ORDER BY `ID` ASC");
    $table.='<tr>
                <th>Receipt No</th>
                <th>Date</th>
                <th>Debit</th>
    ...
    foreach ($collection as $k => $valu) {
        if($valu['amount_type']=='D'){
            ...
            $balance=$totDebit-$totDiscount-$totCredit;
            ...
            $totDebit=$objMisc->GiveValue(" AMOUNT_TYPE='D' AND SUBSCRIBER_ID='$id' AND ID<='$valu[id]' AND STATUS='A'",'SUM(AMOUNT)','monthly_charges');
            $credit==$valu['amount'];
            $monthDate=$valu['month_date'];
