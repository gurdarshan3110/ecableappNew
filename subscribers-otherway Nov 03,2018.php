...
$wherethis = " 1=1 AND S.HEADOFFICE_ID='$_SESSION[HEADOFFICE]' $and ORDER BY S.`CUSTOMER_ID` ASC";
...
function subInfo($id){
    ...
    $table.='<table style="width:100%" class="table  table-bordered table-striped">';
        ...
        $rec=$objMisc->getAllRecordsNew("SELECT B.STB_NO,P.NAME,P.PARENT_CHARGES,S.* FROM `stb_box` S JOIN `subscriptions` B ON S.SUBSCRIPTION_ID=B.SUBSCRIPTION_ID JOIN `package_master` P ON P.PACKAGE_ID=S.PACKAGE_ID WHERE SUBSCRIBER_ID='$id'");
        $q=1;
        ...
        foreach ($rec as $k => $value) {
            ...
            if($value['status']=='A'){
                $table.='<tr style="color:#005E00;">
                            ...
                            <td>'.$value['name'].'</td>
                            <td>'.(($value['la_carte_id']==0)?'Not Applicable':$laCartePackage['name']).'</td>
                            <td style="text-align:right;">'.($value['parent_charges']+$laCartePackage['parent_charges']).'</td>
                        </tr>';
                $totCharges=$totCharges+$value['parent_charges']+$laCartePackage['parent_charges'];
            }else{
                $table.='<tr style="color:#D20000;">
                            <td>'.$q.'</td>
                            <td>'.$value['stb_no'].'</td>
                            <td>'.$value['identity_name'].'<br>'.$value['relation'].' '.$value['relative'].'</td>
                            <td>'.$value['installed_by'].(($value['installation_fees']!='')?'<br>Fees '.$value['installation_fees']:'').(($value['joining_date']!='0000-00-00 00:00:00')?'<br>Date '.$objMisc->DateFormat($value['joining_date']):'').'</td>
