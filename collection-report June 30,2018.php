...
$wherethis = " 1=1 AND MC.HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND MC.STATUS='A' AND MC.`AMOUNT_TYPE`='C' $and ORDER BY date(MC.`MONTH_DATE`) ASC";
$sql="SELECT S.`CUSTOMER_ID`,S.`PHONE_NO`,S.`NAME`,S.`ADDRESS`,(SELECT SUM(`DISCOUNT`) FROM `monthly_charges` WHERE SUBSCRIBER_ID=S.`SUBSCRIBER_ID` AND STATUS='A' AND AMOUNT_TYPE='C' $and)AS `DISCOUNT`,(SELECT SUM(`AMOUNT`) FROM `monthly_charges` WHERE SUBSCRIBER_ID=S.`SUBSCRIBER_ID` AND AMOUNT_TYPE='C' $and)AS `COLLECTION`,((SELECT SUM(`AMOUNT`) FROM `monthly_charges` WHERE SUBSCRIBER_ID=S.`SUBSCRIBER_ID` AND STATUS='A' AND AMOUNT_TYPE='D' $and)-(SELECT SUM(`AMOUNT`) FROM `monthly_charges` WHERE SUBSCRIBER_ID=S.`SUBSCRIBER_ID` AND STATUS='A' AND AMOUNT_TYPE='C' $and)-(SELECT SUM(`DISCOUNT`) FROM `monthly_charges` WHERE SUBSCRIBER_ID=S.`SUBSCRIBER_ID` AND STATUS='A' AND AMOUNT_TYPE='C' $and))AS BALANCE,(SELECT SUM(`AMOUNT`) FROM `monthly_charges` WHERE SUBSCRIBER_ID=S.`SUBSCRIBER_ID` AND STATUS='A' AND AMOUNT_TYPE='D' $and)AS PENDING FROM `subscribers` S JOIN `monthly_charges` MC ON S.`SUBSCRIBER_ID`=MC.`SUBSCRIBER_ID` WHERE $wherethis";
