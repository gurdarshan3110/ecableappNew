...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    ...
    $and ='';
    $and1='';
    if(!empty($_POST['wing_id'])){
       $wing_id      = $_POST['wing_id'];
       $and         .= " AND W.WING_ID='$wing_id'"; 
       $and2         .= " AND W.WING_ID='$wing_id'"; 
       $and1        .= " AND WING_ID='$wing_id'"; 
    }
    if(!empty($_POST['unit_id'])){
       $unit_id     = $_POST['unit_id'];
       $and        .= " AND S.UNIT_ID='$unit_id'"; 
       $and2        .= " AND U.UNIT_ID='$unit_id'"; 
       $and1       .= " AND UNIT_ID='$unit_id'"; 
    }
    $totalSubscribers   = $objMisc->GiveValueNew("SELECT count(*) FROM `subscribers` S JOIN `unit_master` U ON U.`UNIT_ID`=S.`UNIT_ID` JOIN `wing_master` W ON W.`WING_ID`=U.`WING_ID` JOIN `headoffice_master` H ON H.`HEADOFFICE_ID`=W.`HEADOFFICE_ID` WHERE S.`STATUS`='A' AND S.`HEADOFFICE_ID`='$_SESSION[HEADOFFICE]' $and");
    $subscribers=" SELECT S.`SUBSCRIBER_ID`,S.`AMOUNT` FROM `subscribers` S JOIN `unit_master` U ON U.`UNIT_ID`=S.`UNIT_ID` JOIN `wing_master` W ON W.`WING_ID`=U.`WING_ID` JOIN `headoffice_master` H ON H.`HEADOFFICE_ID`=W.`HEADOFFICE_ID` WHERE S.`STATUS`='A' AND S.`HEADOFFICE_ID`='$_SESSION[HEADOFFICE]' $and";
    $sql=$objMisc->getAllRecordsNew($subscribers);
    //echo "SELECT count(*) FROM `subscribers` S JOIN `monthly_charges` M ON M.`SUBSCRIBER_ID`=S.`SUBSCRIBER_ID` JOIN `unit_master` U ON U.`UNIT_ID`=S.`UNIT_ID` JOIN `wing_master` W ON W.`WING_ID`=U.`WING_ID` JOIN `headoffice_master` H ON H.`HEADOFFICE_ID`=W.`HEADOFFICE_ID` WHERE S.`STATUS`='A' AND S.`USER_ID`='$_SESSION[USER_ID]' AND H.`HEADOFFICE_ID`='$headoffice_id' AND MONTH(`MONTH_DATE`)=month(`$date`) $and";
    $checkExistance = $objMisc->GiveValueNew("SELECT count(*) FROM `subscribers` S JOIN `monthly_charges` M ON M.`SUBSCRIBER_ID`=S.`SUBSCRIBER_ID` JOIN `unit_master` U ON U.`UNIT_ID`=S.`UNIT_ID` JOIN `wing_master` W ON W.`WING_ID`=U.`WING_ID` JOIN `headoffice_master` H ON H.`HEADOFFICE_ID`=W.`HEADOFFICE_ID` WHERE S.`STATUS`='A' AND S.`HEADOFFICE_ID`='$_SESSION[HEADOFFICE]' AND MONTH(`MONTH_DATE`)=month('$date') $and");
    if(empty($checkExistance)){
        foreach ($sql as $k => $row) {
            $subscriber_id  = $row['subscriber_id'];
            $lastMonthDebitVal    = 0;
            $lastMonthCreditVal   = 0;
            $lastMonthVal         = 0;
            $lastMonthDebitVal    = $objMisc->GiveValue(" HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND SUBSCRIBER_ID='$subscriber_id' AND AMOUNT_TYPE='D'",'SUM(AMOUNT)','monthly_charges');
            $lastMonthCreditVal   = $objMisc->GiveValue(" HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND SUBSCRIBER_ID='$subscriber_id' AND AMOUNT_TYPE='C'",'SUM(AMOUNT)','monthly_charges');
            $lastMonthVal         = $lastMonthDebitVal-$lastMonthCreditVal;
            $amount=0;
            $amount         = $row['amount']+$lastMonthVal;
        ...
        exit;
    }else if($checkExistance!=$totalSubscribers){
        $sql = " SELECT S.`SUBSCRIBER_ID`,S.`AMOUNT` FROM `subscribers` S JOIN `unit_master` U ON U.`UNIT_ID`=S.`UNIT_ID` JOIN `wing_master` W ON W.`WING_ID`=U.`WING_ID` JOIN `headoffice_master` H ON H.`HEADOFFICE_ID`=W.`HEADOFFICE_ID` WHERE S.`STATUS`='A' AND S.`HEADOFFICE_ID`='$_SESSION[HEADOFFICE]' $and ORDER BY S.`SUBSCRIBER_ID` DESC LIMIT $checkExistance,$totalSubscribers";
        foreach ($sql as $k => $row) {
            $subscriber_id  = $row['subscriber_id'];
            $amount         = $row['amount'];
            $check          = $objMisc->GiveValue(" HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND SUBSCRIBER_ID",'ID','monthly_charges');
            if(empty($check)){
                $rowArray       = array('HEADOFFICE_ID'           =>$_SESSION['HEADOFFICE'],
                                        'SUBSCRIBER_ID'     =>$subscriber_id,
                                        'AMOUNT_TYPE'       =>'D',
