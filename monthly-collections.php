    ...
    if($canBill=='Y'){    
        if($_POST['typo']=='B'){
            ...
            if($currentDate==date('Y-m-d')){
                $sno=$objMisc->GiveValue("FRANCHISE_ID='$franchiseId' AND AMOUNT_TYPE='C' AND DATE(MONTH_DATE)='$currentDate'",'MAX(SNO)','monthly_charges')+1;
                ...
                $sno=$objMisc->GiveValue("FRANCHISE_ID='$franchiseId' AND AMOUNT_TYPE='C'",'MAX(SNO)','monthly_charges')+1;
            }
            if($currentMonth<=3){
                $session=(date('y')-1).'-'.date('y');
            }else{
            ...
            if($balSms>0 && $amount>0){           
                ...
                $franchiseName=$objMisc->GiveValue("FRANCHISE_ID='$franchiseId'",'NAME','franchise_master');
                $msg="Thanks for ".$franchiseName." subscription Rs ".$amount." against Vr. No. ".$receipt_no." Dated  ".$date." And your balance is ".$amtBal.",Thanks ecableApp ";
