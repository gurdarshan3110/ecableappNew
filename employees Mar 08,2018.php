...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    ...
    if($id){
                ...
                $assignedAgents=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND `IMEI`!=''",'COUNT(`IMEI`)','users');
                if($assignedAgents<$collectionAgents){
                    $rowArray2 = array( 'IMEI'            => $_POST['imei'],
        ...
        if($recdata!='')
        {
            ...
            $assignedAgents=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]'",'COUNT(`IMEI`)','users');
            if($assignedAgents<$collectionAgents){
...
$wherethis = " 1=1 AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]' ORDER BY `EMPLOYEE_ID` DESC";
//echo "SELECT E.EMPLOYEE_ID,E.NAME,E.PHONE_NO,E.ADDRESS,E.STATUS,U.USERNAME,U.PASSWORD FROM employees E JOIN users U ON E.EMPLOYEE_ID=U.ID WHERE U.USER_TYPE='E' AND U.HEADOFFICE_ID='$_SESSION[HEADOFFICE]' ORDER BY E.`NAME` ASC";
$classRecord = $objMisc->getAllRecordsPagingNew("SELECT E.EMPLOYEE_ID,E.NAME,E.PHONE_NO,E.ADDRESS,E.STATUS,U.USERNAME,U.PASSWORD,U.`IMEI` FROM employees E JOIN users U ON E.EMPLOYEE_ID=U.ID WHERE U.USER_TYPE='E' AND U.HEADOFFICE_ID='$_SESSION[HEADOFFICE]' ORDER BY E.`NAME` ASC");
$i  = 1;
$pagin_recs = "";
$pagin_recs = '<input type="hidden" value="" id="checkStatus" name="checkStatus"><thead><tr><th width="20%">Name</th><th width="15%">Phone No/Imei</th><th width="25%">Address</th><th width="10%">Username</th><th width="10%">Password</th><th>Action';
    ...
    { $i=1;
       foreach ($classRecord[1] as $k => $rowRec)
        {       
            ...
            $pagin_recs .='<td>'.$rowRec['phone_no'].'<br>'.$rowRec['imei'].'</td>';
...
function deleteRow($id)
{
    ...
    foreach($idArray as $bid)
    {
       ...
       $objMisc->delete("employees",$where); 
       $where1 = " ID ='$bid' AND USER_TYPE='E'";
       $objMisc->delete("users",$where1); 
    }
    $_SESSION['msg'] = 3;
    $sscript .= "var a;";
