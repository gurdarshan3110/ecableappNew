...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    ...
    //$subscribers=" SELECT S.`SUBSCRIBER_ID`,B.`PACKAGE_ID`,P.`PARENT_CHARGES` FROM `subscribers` S JOIN `unit_master` U ON U.`UNIT_ID`=S.`UNIT_ID` JOIN `wing_master` W ON W.`WING_ID`=U.`WING_ID` JOIN `headoffice_master` H ON H.`HEADOFFICE_ID`=W.`HEADOFFICE_ID` JOIN `stb_box` B ON S.`SUBSCRIBER_ID`=B.`SUBSCRIBER_ID` JOIN `package_master` P ON P.`PACKAGE_ID`=B.`PACKAGE_ID` WHERE  B.`STATUS`='A' AND S.`HEADOFFICE_ID`='$_SESSION[HEADOFFICE]' $and";
    $subscribers="SELECT B.SUBSCRIBER_ID,B.`PACKAGE_ID`,P.`PARENT_CHARGES` FROM `stb_box` B JOIN `package_master` P ON P.`PACKAGE_ID`=B.`PACKAGE_ID` WHERE  B.`STATUS`='A' AND B.`HEADOFFICE_ID`='$_SESSION[HEADOFFICE]'";
    ...
    if(empty($checkExistance)){
        ...
        foreach ($sql as $k => $row) {
            $subscriber_id  = $row['subscriber_id'];
            $amount         = $row['parent_charges'];
            $check          = $objMisc->GiveValue(" HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND SUBSCRIBER_ID='$subscriber_id' AND AMOUNT_TYPE='D'",'ID','monthly_charges');
            if(empty($check)){
                $rowArray       = array('HEADOFFICE_ID'           =>$_SESSION['HEADOFFICE'],
                                        'SUBSCRIBER_ID'     =>$subscriber_id,
                                        'AMOUNT_TYPE'       =>'D',
