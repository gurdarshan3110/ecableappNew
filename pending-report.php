...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['submit']) || $_REQUEST['method']=='search'){
    ...
    if(!empty($_REQUEST['unit_id'])){
        $andWhere .=" AND S.UNIT_ID='$_REQUEST[unit_id]'"; 
...
$monthCurrent=date('m');
$yearCurrent=date('Y');
//echo "SELECT (SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE SUBSCRIBER_ID=S.SUBSCRIBER_ID AND AMOUNT_TYPE='D' AND STATUS='A')as totdebit,(SELECT IFNULL(SUM(DISCOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A')as totdiscount,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A')as totcredit,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE SUBSCRIBER_ID=S.SUBSCRIBER_ID AND AMOUNT_TYPE='D' AND STATUS='A' AND MONTH(MONTH_DATE)='$monthCurrent')as totdebit1,(SELECT IFNULL(SUM(DISCOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A' AND MONTH(MONTH_DATE)='$monthCurrent')as totdiscount1,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A' AND MONTH(MONTH_DATE)='$monthCurrent')as totcredit1,S.* FROM monthly_charges M JOIN `subscribers` S ON S.SUBSCRIBER_ID=M.SUBSCRIBER_ID WHERE $wherethis";
$classRecord = $objMisc->getAllRecordsPagingNew("SELECT (SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE SUBSCRIBER_ID=S.SUBSCRIBER_ID AND AMOUNT_TYPE='D' AND STATUS='A')as totdebit,(SELECT IFNULL(SUM(DISCOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A')as totdiscount,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A')as totcredit,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE SUBSCRIBER_ID=S.SUBSCRIBER_ID AND AMOUNT_TYPE='D' AND STATUS='A' AND MONTH(MONTH_DATE)='$monthCurrent' AND YEAR(MONTH_DATE)='$yearCurrent')as totdebit1,(SELECT IFNULL(SUM(DISCOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A' AND MONTH(MONTH_DATE)='$monthCurrent' AND YEAR(MONTH_DATE)='$yearCurrent')as totdiscount1,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A' AND MONTH(MONTH_DATE)='$monthCurrent' AND YEAR(MONTH_DATE)='$yearCurrent')as totcredit1,S.* FROM monthly_charges M JOIN `subscribers` S ON S.SUBSCRIBER_ID=M.SUBSCRIBER_ID WHERE $wherethis");
...
if(is_array($classRecord[1]) && !empty($classRecord[1])){ 
    ...
    foreach ($classRecord[1] as $k => $rowRec){    
        //echo$j."-SUBSCRIBER_ID='$rowRec[subscriber_id]' AND STATUS='A' AND AMOUNT_TYPE='D' AND MONTH(MONTH_DATE)<='$lastMonth' AND YEAR(MONTH_DATE)='$lastYear'".'<br>';
        ...
        $pagin_recs .='<td>'.trim($rowRec['address']).'</td>';
