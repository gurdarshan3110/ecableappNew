...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    ...
    if($canBill=='Y'){    
        if($_POST['typo']=='B'){
            ...
            $phones=$objMisc->GiveValue("FRANCHISE_ID='$franchiseId'",'MOBILE_NO','franchise_master');
            $currentMonth=date('m');
            if($currentMonth<=3){
            ...
            $sno=$objMisc->GiveValue("FRANCHISE_ID='$franchiseId' AND AMOUNT_TYPE='C' AND DATE(MONTH_DATE) BETWEEN '$startDate' AND '$endDate'",'MAX(SNO)','monthly_charges')+1;
            ...
            //$receipt_no1=$receipt_no;
            //$receipt_no=((!empty($_POST['manual']))?$_POST['manual']:$receipt_no);
            $lastId=mysqli_insert_id($_SESSION['CONN']);
            if($balSms>0 && $amount>0){           
                ...
                $franchiseName=$objMisc->GiveValue("FRANCHISE_ID='$franchiseId'",'NAME','franchise_master');
                $msg="Thanks for ".$franchiseName." subscription Rs ".$amount." against Vr. No. ".$receipt_no." Dated  ".$date." And your balance is ".$amtBal.".Complaint No. ".$phones;
...
switch ($msg)
{
    ...
    break;
     case 7:
        $errormsg = "Please Top Up your Wallet to continue Collections.";
        $msg = "";
    break;
}
...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['search'])){
    if(!empty($_POST['collection_date'])){
        $collection_date=$objMisc->changeDateFormat($_POST['collection_date']);
        $and.=" AND MONTH_DATE='$collection_date'";
        $paging_params.="&collection_date=".$_POST['collection_date'];
    }
    if(!empty($_POST['subscriber_id'])){
        $and.=" AND SUBSCRIBER_ID='$_POST[subscriber_id]'";
        $paging_params.="&subscriber_id=".$_POST['subscriber_id'];
    }
    if(!empty($_POST['receipt_no'])){
        $and.=" AND (RECEIPT_NO LIKE '$_POST[receipt_no]%' OR MANUAL_RECEIPT_NO LIKE '$_POST[receipt_no]%')";
        $paging_params.="&receipt_no=".$_POST['receipt_no'];
    }
...
$wherethis = " 1=1 AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]' $and AND AMOUNT_TYPE='C' ORDER BY `ID` DESC";
$classRecord = $objMisc->getAllRecordsPaging("*","monthly_charges",$wherethis);
$i  = 1;
$pagin_recs = "";
$pagin_recs = '<input type="hidden" value="" id="checkStatus" name="checkStatus"><thead><tr><th width="10%">Receipt No</th><th width="10%">Manual Receipt No</th><th width="35%">Subscriber</th><th width="10%">Phone No</th><th width="15%">Collection</th><th>Action';
/*
    ...
    { $i=1;
       foreach ($classRecord[1] as $k => $rowRec)
        {       
            ...
            $pagin_recs .='<td align="right">'.$rowRec['amount'].'<br>'.$objMisc->dateFormat($rowRec['month_date']).' '.date('H:i A',strtotime($rowRec['added_time'])).'</td>';
            //$pagin_recs .='<!--<td><a class="btn btn-info" target="_blank" href="receipt-pdf-.php?id='.$rowRec['id'].'">Print</a>-->';
            $pagin_recs .='<td><a class="btn btn-info btn-small" target="_blank" href="receipt-pdf.php?id='.$rowRec['id'].'">Receipt</a>&nbsp;<!--<a class="btn btn-primary btn-small" target="_blank" href="invoice-pdf.php?id='.$rowRec['id'].'">Invoice</a>--><input type="hidden" name="enumStatus'.$rowRec['id'].'" id="enumStatus'.$rowRec['id'].'"  value="'.$rowRec['status'].'">&nbsp;<b style="color:red;" id="action'.$rowRec['id'].'">';
           // echo $_SESSION['USER_ID'];
                if($rowRec['status']=='A'){
                    $pagin_recs .= '<input type="image" src="images/active-btn.gif" name="imgStatus'.$rowRec['id'].'" id="imgStatus'.$rowRec['id'].'" alt="Cancel Receipt" title="Cancel Receipt" border="0" onclick="return updateStatus('.$rowRec['id'].');">&nbsp;';
                }else{
