...
switch($action)
{
    ...
    $totalDebit     =   $objMisc->GiveValue($where." AND AMOUNT_TYPE='D'",'SUM(`AMOUNT`)','monthly_charges');
    $totalCredit    =   $objMisc->GiveValue($where." AND AMOUNT_TYPE='C'",'SUM(`AMOUNT`)','monthly_charges');
    ...
    $name = $_REQUEST['name'];
        ...
        $query2=mysql_query("SELECT subscriber_id,name,customer_id FROM subscribers where STATUS='A' and HEADOFFICE_ID='$_SESSION[HEADOFFICE]' $or ORDER BY SUBSCRIBER_ID DESC");
        echo "<ul>";
