...
switch($action)
{
    ...
    $totalDiscount    =   $objMisc->GiveValue($where." AND STATUS='A' AND AMOUNT_TYPE='C'",'SUM(`DISCOUNT`)','monthly_charges');
    $totalCredit=$totalCredit+$totalDiscount;
    ...
    $name = $_REQUEST['name'];
        ...
        $query2=$objMisc->getAllRecordsNew("SELECT subscriber_id,name,customer_id,mso_id FROM subscribers where STATUS='A' and HEADOFFICE_ID='$_SESSION[HEADOFFICE]' $or ORDER BY MSO_ID,NAME,CUSTOMER_ID ASC");
        echo "<ul>";
        if(!empty($query2)){
        foreach($query2 as $r =>$query3)
        {
           ?>
          <li onclick='fill("<?php echo $query3['name'].' ('.$query3['customer_id'].')';?>");fillUser(<?php echo $query3['subscriber_id'];?>);fetchbill();getSubscriberData();'><b><?php echo $query3['name'].' (Customer Id: '.$query3['customer_id'].')'.' (Customer Code: '.$query3['mso_id'].')';?></li>
          <?php
        }
        }
        else{
    ...
    $name = $_REQUEST['name'];
        ...
        {
           ?>
          <li onclick='fill1("<?php echo $query3['name'].' ('.$query3['customer_id'].')';?>");fillUser1(<?php echo $query3['subscriber_id'];?>);fetchbill();getSubscriberData();'><b><?php echo $query3['name'].' (Customer Id: '.$query3['customer_id'].')'.' (Customer Code: '.$query3['mso_id'].')';?></li>
          <?php
        }
        }
        else{
