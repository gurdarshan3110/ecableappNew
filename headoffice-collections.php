...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    $exist=$objMisc->GiveValue("HEADOFFICE_ID='$_POST[headoffice_id]'",'COLLECTION_ID','admin_collections');
    $rowArray = array(	'HEADOFFICE_ID'     => $_POST['headoffice_id'],
                        'RATE_SMS'          => $_POST['rate_sms'],
                        'RATE'              => $_POST['rate'],
                        'CREDIT_AMOUNT'     => $_POST['credit_amount'],
                        'TYPE'              => $_POST['type'],
                        'ADDED_TIME'        => date('Y-m-d H:i:s'));
    if(!empty($exist)){
        $insert=$objMisc->update('admin_collections',$rowArray,"COLLECTION_ID='$exist'");
    }else{
        $insert=$objMisc->insert('admin_collections',$rowArray);
    }
    $_SESSION['msg'] = 1;
    header("location:headoffice-collections.php");
    exit;
}
...
if($id){
    ...
    $rowArray = $objMisc->getRow("admin_collections",$where);
    //$rowArray1 = $objMisc->getRow("admin_collections",$where);

}
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['search'])){
    if(!empty($_POST['headoffice_id'])){
        $and .=" AND HEADOFFICE_ID='$_POST[headoffice_id]'";
    }
    
    $search=$_POST;
}
$wherethis = " 1=1 $and ORDER BY `COLLECTION_ID` DESC";
$classRecord = $objMisc->getAllRecordsPaging("*","admin_collections",$wherethis);
$i  = 1;
$pagin_recs = "";
$pagin_recs = '<input type="hidden" value="" id="checkStatus" name="checkStatus"><thead><tr><th width="50%">Headoffice</th><th width="10%">Rate/Sms</th><th width="10%">Rate/Bill</th><th width="10%">Credit Amount</th><th width="15%">Date</th><th>Action';
    ...
    { $i=1;
       foreach ($classRecord[1] as $k => $rowRec)
        {       
            ...
            $pagin_recs .='<td style="text-align:right;">'.$rowRec['rate_sms'].'</td>';
            $pagin_recs .='<td style="text-align:right;">'.$rowRec['rate'].'</td>';
            $pagin_recs .='<td style="text-align:right;">'.$rowRec['credit_amount'].'</td>';
            $pagin_recs .='<td>'.$objMisc->dateFormat($rowRec['added_time']).'</td>';
           ...
           // echo $_SESSION['USER_ID'];
                    $pagin_recs .='<a href="headoffice-collections.php?id='.base64_encode($rowRec['collection_id']).'&page='.$_GET['page'].'" ><img src="images/edit-btn.gif"  alt="Edit" title="Edit"  border="0"></a>&nbsp;';
                    if($rowRec['status']=='A')
                    {
                       $pagin_recs .= '<input type="image" src="images/active-btn.gif" name="imgStatus'.$rowRec['collection_id'].'" id="imgStatus'.$rowRec['collection_id'].'" alt="Deactivate" title="Deactivate" border="0" onclick="return myChangeStatus(this.form,'.$rowRec['collection_id'].');">&nbsp;';
                    }
                    else
                    {
                       $pagin_recs .= '<input type="image" src="images/deactive-btn.gif" name="imgStatus'.$rowRec['collection_id'].'" id="imgStatus'.$rowRec['collection_id'].'" alt="Activate" title="Activate" border="0" onclick="return myChangeStatus(this.form,'.$rowRec['collection_id'].');">&nbsp;';
                    }
...
$where     =   " 1 = 1 order by HEADOFFICE_ID";
$ofcArray  =   $objMisc->myFunc->fnWriteOptionList($rowArray['headoffice_id'],'NAME,HEADOFFICE_ID','headoffice_master',$where,0,0,0); 
$searchArray  =   $objMisc->myFunc->fnWriteOptionList($search['headoffice_id'],'NAME,HEADOFFICE_ID','headoffice_master',$where,0,0,0); 
$smartyVars['searchArray']     =   $searchArray;
$smartyVars['headofficeArray']     =   $ofcArray;
...
$objMisc->displayPage("header,headoffice-collections,footer",$smartyVars);
function update_status($id,$status)
{
    ...
    $where = "COLLECTION_ID =".$id;
    $objMisc->update("admin_collections",$row,$where);
