...
if(!isset($_SESSION['USER_ID'])){
  ...
  ?>
    ...
    <div class="start-meeting" id="start-meeting" onclick="startMeeting();"><img src="https://v2.konspot.com/images/start-meeting.png" style="width: 90px;position: absolute;bottom: 1%;right: 1%;cursor:pointer;" id="openMeeting" class="openMeeting"/></div>
    {literal}
    <script type="text/javascript">var api="mnutrGkfKJo4uNZtnCLVV";var roomkey="fDnycpIClI";</script>
    <script type="text/javascript" src="https://v2.konspot.com/js/konspot-widgets-temp.js"></script>
    {/literal}
  <?php
  include("footer-frontend.php");
    ...
    if($_SESSION['USER_TYPE']=='S'){
    	$wherethis = " 1=1 AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND SUBSCRIBER_ID='$_SESSION[SUBSCRIBER_ID]' AND STATUS='A' ORDER BY `ID` DESC";
        ...
        { $i=1;
           foreach ($classRecord[1] as $k => $rowRec)
            {   
                if($i==1){
                    $balance=$bal;
                    $_SESSION['transaction_Amt']=$balance;
                    $link=(($bal>0 && ($_SESSION['HEADOFFICE']=='18' || $_SESSION['HEADOFFICE']=='9'))?"<a href='/icici/pay.php' class='btn btn-success'>Pay ".number_format($balance,2)."</a>":(($bal>0 && $_SESSION['HEADOFFICE']=='7')?"<a href='/paytmLive/webpay.php' class='btn btn-success'>Pay ".number_format($balance,2)."</a>":''));
                }else{
                ...
                $pagin_recs .='<td style="text-align:right;">'.(($rowRec['amount_type']=='C')?'<a href="receipt-pdf.php?id='.$rowRec['id'].'" title="Click for PDF Bill Copy" target="_blank">':'').number_format($debit,2).(($rowRec['amount_type']=='C')?'</a>':'').'</td>
                        ...
                        <td style="text-align:right;">'.(($rowRec['amount_type']=='C')?'<a href="receipt-pdf.php?id='.$rowRec['id'].'" title="Click for PDF Bill Copy" target="_blank">':'').(($i==1 && $balance>0 && $link!='')?$link:number_format($balance,2)).(($rowRec['amount_type']=='C')?'</a>':'').'</td>';
    ...
    $objMisc->displayPage("header,index-widget1,footer-widget",$smartyVars);
}
?>