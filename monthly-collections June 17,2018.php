...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    if($_POST['typo']=='B'){
        ...
        if($balSms>0){
            ...
            $msg="Payment of ".$amount." has been recieved for connection of ".$_SESSION['HEADOFFICE_NAME'];
...
$i  = 1;
$pagin_recs = "";
$pagin_recs = '<input type="hidden" value="" id="checkStatus" name="checkStatus"><thead><tr><th width="25%">Subscriber</th><th width="25%">Address</th><th width="15%">Collection</th><th width="15%">Date</th><th>Action';
/*
    ...
    { $i=1;
       foreach ($classRecord[1] as $k => $rowRec)
        {       
            ...
            $pagin_recs .='<td>'.$objMisc->GiveValue(" SUBSCRIBER_ID='$rowRec[subscriber_id]'",'NAME','subscribers').'</td>';
            $headoffice_id =$objMisc->GiveValue(" _ID='$rowRec[wing_id]'",'HEADOFFICE_ID','wing_master');
            $pagin_recs .='<td>'.$objMisc->GiveValue(" SUBSCRIBER_ID='$rowRec[subscriber_id]'",'ADDRESS','subscribers').'</td>';
            ...
            $pagin_recs .='<td><a class="btn btn-info">Print</a><input type="hidden" name="enumStatus'.$rowRec['id'].'" id="enumStatus'.$rowRec['id'].'"  value="'.$rowRec['status'].'">&nbsp;<b style="color:red;" id="action'.$rowRec['id'].'">';
...
$receipt=$objMisc->GiveValues("1=1 ORDER BY ID DESC LIMIT 1",'RECEIPT_NO,MANUAL_RECEIPT_NO','monthly_charges');
$receipt_no=((!empty($receipt['receipt_no']))?$receipt['receipt_no']+1:'1');
