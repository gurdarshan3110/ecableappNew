...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    ...
    $subscribers=" SELECT B.`SUBSCRIBER_ID`,B.`PACKAGE_ID`,B.`LA_CARTE_AMOUNT`,P.`PARENT_CHARGES`,B.STB_ID FROM `stb_box` B JOIN `package_master` P ON P.`PACKAGE_ID`=B.`PACKAGE_ID` JOIN `subscribers` S ON S.SUBSCRIBER_ID=B.SUBSCRIBER_ID WHERE  B.`STATUS`='A' AND S.`STATUS`='A' AND B.`HEADOFFICE_ID`='$_SESSION[HEADOFFICE]'";
    ...
    $subscriberCount=0;
        foreach ($sql as $k => $row) {
            ...
            $checkExistance=$objMisc->GiveValue("SUBSCRIBER_ID='$subscriber_id' AND MONTH(MONTH_DATE)='$month' AND STB_ID='$stb_id' AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]'",'ID','monthly_charges');
            if (empty($checkExistance)) {
                $subscri_id  = $row['subscriber_id'];
                $unit_id=$objMisc->GiveValue("SUBSCRIBER_ID='$subscri_id' AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]'",'UNIT_ID','subscribers');
