        ...
        if (!$fp = fopen($fileName,"r")){
            ...
            while(!feof($fp)) {
                while (($data = fgetcsv($fp, 10000, ",")) !== FALSE) { 
                    ...
                    //echo " HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND STB_NO='$data[1]'";exit;
                    ...
                    if($subscription==''){
                        $subscriptionArray = array( 'HEADOFFICE_ID'          =>$_SESSION['HEADOFFICE'],
                                                    'STB_NO'        =>(($data[1]!='')?addslashes($data[1]):'DUMMY-'.$subscriberId.'-'.rand(1,99)),
                        ...
                        $subscriptionArray = array( 'AVAILABILITY'  =>'B');
                        $insertSubscription=$objMisc->update('subscriptions',$subscriptionArray,"SUBSCRIPTION_ID='$subscription' AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]'");
                    }
                    ...
                    if ($data[6]!='') {
                        ...
                        $setupBoxArray = array(     'HEADOFFICE_ID'    =>$_SESSION['HEADOFFICE'],
                                                    ...
                                                    'LA_CARTE_AMOUNT'  =>(($data[17]=='')?0:$data[17]),
                                                    'ID_NO'            =>$data[8],
                                                    'IDENTITY_NAME'    =>$data[9],
                                                    'RELATION'         =>$data[10],
                                                    'RELATIVE'         =>$data[11],
                                                    'INSTALLED_BY'     =>$data[12],
                                                    'BOX_CHARGES'      =>(($data[13]=='')?0:$data[13]),
                                                    'JOINING_DATE'     =>(($data[14]=='')?date('Y-m-d'):$objMisc->changeDateFormat($data[14])),
                                                    'TYPE_ID'          =>$data[7],
                                                    'REMARKS'=>$data[21].(($data[20]!='')?' Deactivated on '.$data[20]:''),
                                                    'ADDED_BY'         =>'1',
