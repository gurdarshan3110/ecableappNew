        ...
        if (!$fp = fopen($fileName,"r")){
            ...
            while(!feof($fp)) {
                while (($data = fgetcsv($fp, 10000, ",")) !== FALSE) { 
                    ...
                    $package_id=$objMisc->GiveValue(" HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND PACKAGE_TYPE='P' AND NAME='$data[8]'",'PACKAGE_ID','package_master');
                    if($package_id=='' && ($data[11]!='' || $data[11]!=0)){
                        $packageArray = array( 'HEADOFFICE_ID'  =>$_SESSION['HEADOFFICE'],
                                            'NAME'              =>addslashes($data[8]),
                                            'PARENT_CHARGES'    =>$data[7],
                                            'PACKAGE_TYPE'      =>'P',
                                            'ADDED_BY'          =>$_SESSION['USER_ID'],
                                            'ADDED_TIME'        =>date('Y-m-d H:i:s'),
                                            );
                    ...
                    if ($data[6]!='') {
                        ...
                        $laCarteId=$objMisc->GiveValue(" HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND PACKAGE_TYPE='L' AND NAME='$data[10]'",'PACKAGE_ID','package_master');
                        if($laCarteId=='' && ($data[10]!='' || $data[10]!=0)){
                            $packageArray = array( 'HEADOFFICE_ID'  =>$_SESSION['HEADOFFICE'],
