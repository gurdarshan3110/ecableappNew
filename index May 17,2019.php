...
if($_SESSION['USER_TYPE']=='S'){
	...
	$totalDues=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND AMOUNT_TYPE='D' AND STATUS='A' AND MONTH_DATE<='$to'",'SUM(AMOUNT)','monthly_charges');
    $totCollection=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND AMOUNT_TYPE='C' AND STATUS='A' AND MONTH_DATE<='$to'",'SUM(AMOUNT)','monthly_charges');
	$totDiscount=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND AMOUNT_TYPE='C' AND STATUS='A' AND MONTH_DATE<='$to'",'SUM(DISCOUNT)','monthly_charges');
	$totPending=($totalDues-$totCollection-$totDiscount);
	$billsGeneratedRate=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND AMOUNT_TYPE='C'",'SUM(BILL_RATE)','monthly_charges');
    $billsGenerated=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND AMOUNT_TYPE='C'",'COUNT(*)','monthly_charges');
    $smsSent=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND RATE_SMS!='0.00'",'COUNT(*)','sms_log');
