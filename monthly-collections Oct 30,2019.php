...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    if($_POST['typo']=='B'){
        ...
        if($balSms>0){           
             $subscriberDebit=$objMisc->GiveValue(" SUBSCRIBER_ID='$subscriber_id' AND STATUS='A' AND AMOUNT_TYPE='D' AND ID<='$lastId'",'SUM(AMOUNT)','monthly_charges');
            $subscriberCollec=$objMisc->GiveValue(" SUBSCRIBER_ID='$subscriber_id' AND STATUS='A' AND AMOUNT_TYPE='C' AND ID<='$lastId'",'SUM(AMOUNT)','monthly_charges');
            $subscriberDiscount=$objMisc->GiveValue(" SUBSCRIBER_ID='$subscriber_id' AND STATUS='A' AND AMOUNT_TYPE='C' AND ID<='$lastId'",'SUM(DISCOUNT)','monthly_charges');
            $amtBal=$subscriberDebit-$subscriberCollec-$subscriberDiscount;
            $smsGroupId=$sms->max_sms_groupid($_SESSION['HEADOFFICE']);
            $msg="Thanks for cable subscription Rs ".$amount." against Vr. No. ".$receipt_no." Dated  ".$date." And your balance is ".$amtBal." ".$_SESSION['HEADOFFICE_NAME'];
