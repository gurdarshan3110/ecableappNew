...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['submit']) || $_REQUEST['method']=='search'){
    if(!empty($_REQUEST['subscriber_status']) && $_REQUEST['subscriber_status']!=''){
        $andWhere.=" AND STATUS='$_REQUEST[subscriber_status]'";
        $paging_params.="&subscriber_status=".$_REQUEST['subscriber_status'];
    }else{
        $andWhere.=" AND STATUS!='T'";
        $paging_params.="&subscriber_status=".$_REQUEST['subscriber_status'];
    }
    ...
    if(!empty($_REQUEST['unit_id'])){
        $andWhere .=" AND UNIT_ID='$_REQUEST[unit_id]'"; 
    ...
    if(!empty($permission)){
        ...
        $andWhere.=" AND UNIT_ID IN ($permission)";
    }
    $rowArray=$_REQUEST;
    
}
$wherethis = " 1=1 AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]' $andWhere GROUP BY SUBSCRIBER_ID ORDER BY `SERIAL_NO`*1 ASC";
$monthCurrent=date('m');
$classRecord = $objMisc->getAllRecordsPagingNew("SELECT SUBSCRIBER_ID,SERIAL_NO,MSO_ID,CUSTOMER_ID,NAME,ADDRESS,PHONE_NO,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE SUBSCRIBER_ID=`subscribers`.SUBSCRIBER_ID AND AMOUNT_TYPE='D' AND STATUS='A' AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]')as totdebit,(SELECT IFNULL(SUM(DISCOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=`subscribers`.SUBSCRIBER_ID AND STATUS='A' AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]')as totdiscount,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=`subscribers`.SUBSCRIBER_ID AND STATUS='A' AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]')as totcredit FROM `subscribers` WHERE $wherethis");
...
if(is_array($classRecord[1]) && !empty($classRecord[1])){ 
    ...
    foreach ($classRecord[1] as $k => $rowRec){  
        $balance=$rowRec['totdebit']-$rowRec['totcredit']-$rowRec['totdiscount'];
        if($balance>0){
