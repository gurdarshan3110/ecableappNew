...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    ...
    $insert=$objMisc->insert('headoffice_recharge',$rowArray);
    $_SESSION['msg'] = 1;
    header("location:headoffice-recharge.php");
    exit;
}
...
if($id){
    ...
    $rowArray = $objMisc->getRow("headoffice_recharge",$where);
}
...
$wherethis = " 1=1 $and ORDER BY `RECHARGE_ID` DESC";
$classRecord = $objMisc->getAllRecordsPaging("*","headoffice_recharge",$wherethis);
$i  = 1;
$pagin_recs = "";
$pagin_recs = '<input type="hidden" value="" id="checkStatus" name="checkStatus"><thead><tr><th width="40%">Headoffice</th><th width="10%">Sms</th><th width="25%">Description</th><th width="10%">Date</th><th>Action';
    ...
    { $i=1;
       foreach ($classRecord[1] as $k => $rowRec)
        {       
            ...
            $pagin_recs .='<td> <input type="hidden" name="enumStatus'.$rowRec['recharge_id'].'" id="enumStatus'.$rowRec['recharge_id'].'"  value="'.$rowRec['status'].'">&nbsp;';
           // echo $_SESSION['USER_ID'];
                    if($rowRec['status']=='A')
                    {
                       $pagin_recs .= '<input type="image" src="images/active-btn.gif" name="imgStatus'.$rowRec['collection_id'].'" id="imgStatus'.$rowRec['recharge_id'].'" alt="Deactivate" title="Deactivate" border="0" onclick="return myChangeStatus(this.form,'.$rowRec['recharge_id'].');">&nbsp;';
                    }
                    else
                    {
                       $pagin_recs .= '<input type="image" src="images/deactive-btn.gif" name="imgStatus'.$rowRec['collection_id'].'" id="imgStatus'.$rowRec['recharge_id'].'" alt="Activate" title="Activate" border="0" onclick="return myChangeStatus(this.form,'.$rowRec['recharge_id'].');">&nbsp;';
                    }
...
$objMisc->displayPage("header,headoffice-recharge,footer",$smartyVars);
function update_status($id,$status){
    ...
    $where = "RECHARGE_ID =".$id;
    $objMisc->update("headoffice_recharge",$row,$where);
...
function deleteRow($id)
{
    ...
    {
       $where = " RECHARGE_ID =".$bid;
       $objMisc->delete("headoffice_recharge",$where); 
    }
    $_SESSION['msg'] = 3;
    $sscript .= "var a;";
