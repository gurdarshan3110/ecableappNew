                ...
                if(!empty($_POST['package_id'][$k])){
                    ...
                    if($subscription_id=='' && $stb_id==''){
                        ...
                        if($_POST['current_utility']=='Y'){
                            ...
                            $totPack=$parentCharg+$laparentCharg;
                            $currentMon=date('m');
                            $currentYear=date('Y');
...
$wherethis = " 1=1 AND S.HEADOFFICE_ID='$_SESSION[HEADOFFICE]' $and ORDER BY S.`MSO_ID` ASC";
...
$i  = 1;
$pagin_recs = "";
$pagin_recs = '<input type="hidden" value="" id="checkStatus" name="checkStatus"><thead><tr><th width="20%">Name/Phone No</th><th>Customer Id</th><th>Customer Code/MSO</th><th width="15%">Area/Sub Area</th><th width="12%">Stb Boxes</th><th>Action';
    ...
    if($count==0){
        ...
        $subscriberBoxes .='<div class="col-lg-12">
                                    ...
                                    <div class="col-sm-2">
                                        ...
                                        <select id="la_carte_id'.$i.'"  name="la_carte_id[]" onchange="TotAmt('.$i.')" class="form-control">
        ...
        foreach ($subscriberBoxRecs as $key => $recs) {
            ...
            $wherelaCarte      =   " 1 = 1 AND HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND PACKAGE_TYPE='L' and STATUS = 'A' order by PACKAGE_ID DESC";
            $parentCharges=$objMisc->GiveValue("PACKAGE_ID='$recs[la_carte_id]'",'PARENT_CHARGES','package_master');
            ...
            if($recs['status']=='A'){
                ...
                $subscriberBoxes .='<div class="col-lg-12">
                                        ...
                                        <div class="col-sm-2">
                                            <label>A La Carte</label>
                                            <select  disabled  name="la_carte_id[]" onchange="TotAmt('.$i.')" class="form-control">
                                                <option value="">-Not Applicable-</option>
                                                '.$laCarteArray.'
                                            </select>
                                        </div>
...
$totSubscribers=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]'",'COUNT(*)','subscribers');
$totSubscribersBoxesActive=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND STATUS='A'",'COUNT(*)','stb_box');
...
$totBoxesDeactive=$objMisc->GiveValue("HEADOFFICE_ID='$_SESSION[HEADOFFICE]' AND STATUS='D'",'COUNT(*)','stb_box');
...
function subInfo($id){
    ...
    $table.='<table style="width:100%" class="table  table-bordered table-striped">';
        ...
        $table.='<tr>
                    <th colspan="5" style="text-align:center;">STB Detail</th>
                </tr>';
        $rec=$objMisc->getAllRecordsNew("SELECT B.STB_NO,P.NAME,P.PARENT_CHARGES,S.STATUS,S.LA_CARTE_ID FROM `stb_box` S JOIN `subscriptions` B ON S.SUBSCRIPTION_ID=B.SUBSCRIPTION_ID JOIN `package_master` P ON P.PACKAGE_ID=S.PACKAGE_ID WHERE SUBSCRIBER_ID='$id'");
        $q=1;
        $table.='<tr><th>S.No</th><th>Stb No</th><th>Package</th><th>A La Carte</th><th>Charges</th></tr>';
        $totCharges=0;
        foreach ($rec as $k => $value) {
            ...
            if($value['status']=='A'){
                $table.='<tr style="color:#005E00;"><td>'.$q.'</td><td>'.$value['stb_no'].'</td><td>'.$value['name'].'</td><td>'.(($value['la_carte_id']==0)?'Not Applicable':$laCartePackage['name']).'</td><td style="text-align:right;">'.($value['parent_charges']+$laCartePackage['parent_charges']).'</td></tr>';
                $totCharges=$totCharges+$value['parent_charges']+$laCartePackage['parent_charges'];
            }else{
                $table.='<tr style="color:#D20000;"><td>'.$q.'</td><td>'.$value['stb_no'].'</td><td>Package:'.$value['name'].'</td><td>'.(($value['la_carte_id']==0)?'Not Applicable':$laCartePackage['name']).'</td><td style="text-align:right;">'.($value['parent_charges']+$laCartePackage['parent_charges']).'</td></tr>';
            }
            $q++;
        }
        $table.='<tr><th colspan="4" style="text-align:right;">Total</th><th style="text-align:right;">'.$totCharges.'</th></tr>';
    $table.='</table>
            </div></div></div>';
