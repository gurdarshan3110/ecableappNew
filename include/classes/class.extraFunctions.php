...
class ExtraFunctions 
{
	...
	function fnWriteOptionListIds($id,$fldlist,$table,$where=1,$debug=0,$multi=0,$unique=1){
		$this->dbFunc = new dbFunctions();
		if($unique==1)
		 	$query="select distinct $fldlist from $table where $where ";
		else
		...
		else
		{
			...
			for($counter=0;$counter<$numrows;$counter++)
			{
				$keyR=$record[$counter][$key];
				$valR=$record[$counter][$value];
				$valR1=$record[$counter][$value1];
				/*do not allow records with empty key/values*/
				$valFinal=$valR.'_'.$valR1;
				if(!empty($keyR) && !empty($valR))
				{
					$data.="<option value='$valFinal'";
					if($multi==1)
					{
						$idArray=array();
						$idArray=explode(",",$id);
						$data.=(in_array($valFinal,$idArray))? " selected='selected' " : "";
					}
					else
					{
						$data.= ($id==$valFinal)? " selected='selected' " : "";
					}
					$data.=">".stripslashes($keyR);
					$data.="</option>";
				}
 	...
 	function fnWriteOptionListWithJoin($id,$fldlist,$query,$debug=0,$multi=0)
	{
		$this->dbFunc = new dbFunctions();
		$fldArray=explode(",",$fldlist);
		$key=strtolower($fldArray[0]);
	...
	function fnWriteOptionListWithJoinNew($id,$fldlist,$query,$debug=0,$multi=0)
	{
		$this->dbFunc = new dbFunctions();
		$fldArray=explode(",",$fldlist);
		$key=strtolower($fldArray[0]);
	...
	function fnWriteOptionListWithJoinMsg($id,$fldlist,$query,$debug=0,$multi=0)
	{
		$this->dbFunc = new dbFunctions();
		$fldArray=explode(",",$fldlist);
		$key=strtolower($fldArray[0]);
		$key1=strtolower($fldArray[1]);
		$value=strtolower($fldArray[2]);
