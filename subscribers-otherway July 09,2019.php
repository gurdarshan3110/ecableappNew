...
//echo "SELECT S.NAME as subscriber,W.NAME as area,U.NAME as sub_area,S.* FROM subscribers S LEFT JOIN unit_master U ON U.UNIT_ID=S.UNIT_ID LEFT JOIN wing_master W ON W.`WING_ID`=U.`WING_ID` LEFT JOIN headoffice_master H ON H.HEADOFFICE_ID=W.HEADOFFICE_ID WHERE $wherethis"; exit;
       ...
       foreach ($classRecord[1] as $k => $rowRec)
        {       
            ...
            $pagin_recs .='<table class="table table-bordered">';
                ...
                $sql=$objMisc->getAllRecordsNew("SELECT S.STB_NO,S.SUBSCRIPTION_ID,B.STATUS,B.`STB_ID` FROM `stb_box` B JOIN `subscriptions` S ON S.`SUBSCRIPTION_ID`=B.`SUBSCRIPTION_ID` WHERE B.SUBSCRIBER_ID='$rowRec[subscriber_id]' AND (B.`REMARKS`!='Permanent Disconnection' AND B.`REMARKS`!='Box is Faulty.')");
...
function subLedger($id){
    ...
    $collection=$objMisc->getAllrecordsNew("SELECT `MONTH_DATE`,AMOUNT_TYPE,AMOUNT,RECEIPT_NO,DISCOUNT,SPECIAL_REMARKS,`ID` FROM `monthly_charges` WHERE SUBSCRIBER_ID='$id' AND STATUS='A' ORDER BY `ID` ASC");
    $table.='<tr>
                <th>Receipt No</th>
                <th>Date</th>
                <th>Debit</th>
    ...
    foreach ($collection as $k => $valu) {
        if($monthDate==$valu['month_date'] && $amountType==$valu['amount_type'] && $valu['amount_type']=='D'){
            ...
            if($valu['amount_type']=='D'){
                $openingBal=$objMisc->GiveValue(" AMOUNT_TYPE='D' AND SUBSCRIBER_ID='$id' AND MONTH_DATE='$valu[month_date]' AND RECEIPT_NO='Opening Balance' AND STATUS='A'",'SUM(AMOUNT)','monthly_charges');
                ...
                $totDebit=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND STATUS='A' AND `ID`<'$valu[id]' AND AMOUNT_TYPE='D'",'SUM(AMOUNT)','monthly_charges');
                $totCredit=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND STATUS='A' AND `ID`<'$valu[id]' AND AMOUNT_TYPE='C'",'SUM(AMOUNT)','monthly_charges');
                $totDiscount=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND STATUS='A' AND `ID`<'$valu[id]'",'SUM(DISCOUNT)','monthly_charges');
                $balance=$totDebit+$debit-$totDiscount-$totCredit-$credit-$valu['discount'];
                $dataArray['Balance']=$balance;
                if($debit>0){
                    $table.='<tr>
                                <td>'.$valu['receipt_no'].'</td>
                ...
                $debit=(($valu['amount_type']=='D')?$objMisc->GiveValue(" AMOUNT_TYPE='D' AND SUBSCRIBER_ID='$id' AND MONTH_DATE='$valu[month_date]' AND RECEIPT_NO!='Opening Balance' AND STATUS='A'",'SUM(AMOUNT)','monthly_charges'):'');
                $credit=(($valu['amount_type']=='C')?$valu['amount']:'');
                $amountType=$valu['amount_type'];
                $monthDate=$valu['month_date'];
                $totDebit=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND `ID`<'$valu[id]' AND AMOUNT_TYPE='D' AND STATUS='A'",'SUM(AMOUNT)','monthly_charges');
                $totCredit=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND `ID`<'$valu[id]' AND AMOUNT_TYPE='C' AND STATUS='A'",'SUM(AMOUNT)','monthly_charges');
                $totDiscount=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND `ID`<'$valu[id]' AND AMOUNT_TYPE='C' AND STATUS='A'",'SUM(DISCOUNT)','monthly_charges');
                ...
                $amountType=$valu['amount_type'];
                $monthDate=$valu['month_date'];
                $totDebit=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND `ID`<'$valu[id]' AND STATUS='A' AND AMOUNT_TYPE='D'",'SUM(AMOUNT)','monthly_charges');
                $totCredit=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND `ID`<'$valu[id]' AND STATUS='A' AND AMOUNT_TYPE='C'",'SUM(AMOUNT)','monthly_charges');
                $totDiscount=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND `ID`<'$valu[id]' AND STATUS='A' AND AMOUNT_TYPE='C'",'SUM(DISCOUNT)','monthly_charges');
                $balance=$totDebit+$debit-$totDiscount-$totCredit-$credit-$valu['discount'];
                $table.='<tr>
                                <td>'.$valu['receipt_no'].'</td>
                                <td>'.date('d-m-Y',strtotime($valu['month_date'])).'</td>
    ...
    $table.='<table style="width:100%" class="table  table-bordered table-striped">';
        ...
        foreach ($rec as $k => $value) {
            ...
            if($value['status']=='A'){
                ...
                $table.='<tr title="'.$value['remarks'].'" style="color:'.(($value['remarks']=='Box is Faulty.')?'#D20000':(($value['remarks']=='Permanent Disconnection')?'#FFA500':'#0000FF')).';">
                            <td>'.$q.'</td>
                            <td>'.$value['stb_no'].'</td>
                            <td>'.$value['identity_name'].'<br>'.$value['relation'].' '.$value['relative'].'</td>
                            <td>'.$value['installed_by'].(($value['installation_fees']!='')?'<br>Fees '.$value['installation_fees']:'').(($value['joining_date']!='0000-00-00 00:00:00')?'<br>Date '.$objMisc->DateFormat($value['joining_date']):'').'</td>
