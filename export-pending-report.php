...
$monthCurrent=date('m');
$yearCurrent=date('Y');
$classRecord = $objMisc->getAllRecordsNew("SELECT (SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE SUBSCRIBER_ID=S.SUBSCRIBER_ID AND AMOUNT_TYPE='D' AND STATUS='A')as totdebit,(SELECT IFNULL(SUM(DISCOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A')as totdiscount,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A')as totcredit,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE SUBSCRIBER_ID=S.SUBSCRIBER_ID AND AMOUNT_TYPE='D' AND STATUS='A' AND MONTH(MONTH_DATE)='$monthCurrent' AND YEAR(MONTH_DATE)='$yearCurrent')as totdebit1,(SELECT IFNULL(SUM(DISCOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A' AND MONTH(MONTH_DATE)='$monthCurrent' AND YEAR(MONTH_DATE)='$yearCurrent')as totdiscount1,(SELECT IFNULL(SUM(AMOUNT),0) FROM monthly_charges WHERE AMOUNT_TYPE='C' AND SUBSCRIBER_ID=S.SUBSCRIBER_ID AND STATUS='A' AND MONTH(MONTH_DATE)='$monthCurrent' AND YEAR(MONTH_DATE)='$yearCurrent' AND RECEIPT_NO!='Opening Balance')as totcredit1,S.* FROM monthly_charges M JOIN `subscribers` S ON S.SUBSCRIBER_ID=M.SUBSCRIBER_ID WHERE $wherethis");
...
if(!empty($classRecord)){ 
    ...
    foreach ($classRecord as $k => $rowRec){    
        ...
        $dataArray['address']=str_replace(',', " ", trim($rowRec['address']));
