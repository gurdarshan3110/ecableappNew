...
require_once("helper.php");
    ...
    $collectionData=$objMisc->getAllRecordsNew("SELECT C.AMOUNT,C.SGST,C.CGST,C.MONTH_DATE,C.RECEIPT_NO,C.MANUAL_RECEIPT_NO,S.NAME,S.ADDRESS FROM `monthly_charges` C JOIN `subscribers` S ON S.SUBSCRIBER_ID=C.SUBSCRIBER_ID WHERE C.AMOUNT_TYPE='C' AND C.STATUS='A' AND C.ADDED_BY='$emp_id' AND C.ADDED_BY_TYPE='E' AND C.`MONTH_DATE` BETWEEN '$from_date' AND '$to_date'");             
