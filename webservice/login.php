...
switch($method){ 
    case 'login'; 
        if(!empty($user) && !empty($password)){
            ...
            if($record['user_type']=='S'){
                ...
                $pendingAmount=$objMisc->GiveValue(" SUBSCRIBER_ID='$record[id]' AND AMOUNT_TYPE='D' AND STATUS='A'",'SUM(AMOUNT)','monthly_charges')-$objMisc->GiveValue(" SUBSCRIBER_ID='$record[id]' AND AMOUNT_TYPE='C' AND STATUS='A'",'SUM(AMOUNT)','monthly_charges')-$objMisc->GiveValue(" SUBSCRIBER_ID='$record[id]' AND AMOUNT_TYPE='C' AND STATUS='A'",'SUM(DISCOUNT)','monthly_charges');
                $loggedinUser=array(
            ...
            //}else if($record['user_type']=='E' && $record['imei']==$imei){
                ...
                if($rec['status']=='A'){
                    $loggedinUser=array(
                            ...
                            "headoffice_gstin"=> $office['gstin'],
                            "headoffice_mobile_no"=> $office['mobile_no'],
                            "can_update_package"=> $rec['can_update_package'],
                            "franchises"=>$franData
                        );
                    $json= array(
                            'type'=>"1",
    ...
    case 'Subscribers'; 
        ...
        if(!empty($unitId)){
            ...
            $userId=$_REQUEST['user_id'];
            $areaPermissions = $objMisc->GiveValue("USER_ID='$userId'",'PERMISSIONS','users');
            $unitIds=array_column(json_decode($areaPermissions), 'id');
            $or='';
        ...
        if(!empty($headofficeId)){
            $and .=" AND HEADOFFICE_ID='$headofficeId'";
        }
    ...
    case 'UpdateSubscriberProfile';
        ...
        $rowArray=array('NAME'      =>$name,
                        'PHONE_NO'  =>$phone_no,
                        'EMAIL'  =>$email,
                        'ADDRESS'  =>$address,
                    );
        $updateSub=$objMisc->update('subscribers',$rowArray,$where);
        if($updateSub){
            $json= array(
                    'type'=>"1",
                    'flag'=>'success',
