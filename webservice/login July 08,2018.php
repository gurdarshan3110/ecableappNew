...
switch($method){ 
    ...
    case 'Subscribers'; 
        ...
        $where = " 1=1 $and AND STATUS='A'"; 
        $records = $objMisc->getAllRecords('*','subscribers',$where);
            $k = 1;
            foreach($records as $record){   
                $subscriberDebit=$objMisc->GiveValue(" SUBSCRIBER_ID='$record[subscriber_id]' AND AMOUNT_TYPE='D'",'SUM(AMOUNT)','monthly_charges');
                $subscriberCollec=$objMisc->GiveValue(" SUBSCRIBER_ID='$record[subscriber_id]' AND AMOUNT_TYPE='C'",'SUM(AMOUNT)','monthly_charges');
