...
require_once("helper.php");
    ...
    switch ($action) {
        ...
        case 'NewStb';
            ...
            break;
            case 'StbListing';
                $where="B.SUBSCRIBER_ID='$subscriber_id' AND B.HEADOFFICE_ID='$headoffice_id' AND (B.`REMARKS`NOT LIKE 'Permanent Disconnection%' AND B.`REMARKS` NOT LIKE 'Box is Faulty.%') ORDER BY B.`STB_ID` ASC";
                ...
                $sql=$objMisc->getAllRecordsNew("SELECT S.STB_NO,S.SUBSCRIPTION_ID,P.`NAME`,P.`PARENT_CHARGES`,P.`PACKAGE_ID`,B.`STATUS`,B.`REMARKS` FROM `stb_box` B JOIN `package_master` P ON B.`PACKAGE_ID`=P.`PACKAGE_ID` JOIN `subscriptions` S ON S.SUBSCRIPTION_ID=B.SUBSCRIPTION_ID WHERE $where");
                if(!empty($sql)){
