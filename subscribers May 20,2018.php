...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
    ...
    if($id==''){
        ...
        if(!empty($lastId) && !empty($_POST['phone_no'])){
            $userArray=array('USERNAME'          => $_POST['phone_no'],
        ...
        if(!empty($id)){
            foreach ($_POST['subscription_id'] as $k => $rowRe) {
                $subscription_id=$_POST['subscription_id'][$k];
                $stbId=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND STATUS='A' AND SUBSCRIPTION_ID='$subscription_id'",'ID','stb_box');
                if(empty($stbId)){
                    $stbArray   = array(    'HEADOFFICE_ID'         => $_SESSION['HEADOFFICE'],
                                            'SUBSCRIBER_ID'          => $id,
                                            'SUBSCRIPTION_ID'        => $_POST['subscription_id'][$k],
                    ...
                    //print_r($stbArray);exit;
                    $valstb     = $objMisc->update("stb_box",$stbArray,"`ID`='$stbId'");
                }
            }
        }
        $_SESSION['msg'] = 2;
...
$i  = 1;
$pagin_recs = "";
$pagin_recs = '<input type="hidden" value="" id="checkStatus" name="checkStatus"><thead><tr><th width="30%">Name/Phone No/Address</th><th>Customer Id</th><th>Customer Code/MSO</th><th width="16%">Area/Sub Area</th><th width="12%">Stb Boxes</th><th>Action';
    ...
    { $i=1;
       foreach ($classRecord[1] as $k => $rowRec)
        {       
            ...
            $pagin_recs .='<td>'.$rowRec['subscriber'].'<br>'.$rowRec['phone_no'].'<br>'.$rowRec['address'].'</td>';
                ...
                foreach($sql as $f =>$res){
                    $pagin_recs .='<tr>';
                        ...
                        if($res['status']=='A'){
                           ...
                           $pagin_recs .= '<b style="color:red;">Deactivated</b>&nbsp;';
                        }
                        $pagin_recs .='</td>';
                    $pagin_recs .='</tr>';
                    $f++;
           ...
           // echo $_SESSION['USER_ID'];
                ...
                $pagin_recs .='<a href="subscriber-details.php" class="btn btn-info">Info</a>&nbsp;';
    ...
    foreach ($subscriberBoxRecs as $key => $recs) {
        ...
        if($recs['status']=='A'){
            $subscriberBoxes .='<div class="col-lg-12">
                                    <div class="col-sm-4">
                                        <label>Stb NO</label>
                                        <input required type="text" value="'.$recs['stb_no'].'" name="subscription[]" id="subscription_id1'.$i.'" autocomplete="off" class="form-control"/>
                                        <input required type="hidden" value="'.$recs['stb_id'].'" name="subscription_id[]" id="subscription1'.$i.'" autocomplete="off" class="form-control"/>
...
function updateStatus($id,$status,$reason)
{
    ...
    $rowArray = array(
                    ...
                    'REMARKS'=>(($changeTo=='A')?'Stb activated.Reason :'.$reason:'Stb deactivated.Reason :'.$reason),
                    'ADDED_BY'=>$_SESSION['USER_ID'],
                    'ADDED_TIME'=>date('Y-m-d H:i:s')
                );
    ...
    $objResponse->addAssign('status'.$id,'innerHTML','<b style="color:red;">Deactivated</b>');
