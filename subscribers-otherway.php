...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['submit'])){
	...
	if($recdata==$id && $id!='' || $recdata=='' && $id!=''){
		...
		$rowArray = array(  
			'NAME'              => $_POST['name'],
			'DOB'               => $objMisc->changeDateFormat($_POST['dob']),
			'FRANCHISE_ID'      => $_POST['franchise_id'],
			'CUSTOMER_ID'       => $_POST['customer_id'],
			'MSO_ID'            => $_POST['mso_id'],
			'SERIAL_NO'         => $_POST['serial_no'],
			'SUBSCRIBER_GST'    => $_POST['subscriber_gst'],
		...
		if(!empty($id)){
			...
			if(empty($userId) && !empty($_POST['phone_no'])){
				$userArray = array(
					'USERNAME'         => ((empty($_POST['phone_no']))?$_POST['customer_id']:$_POST['phone_no']),
				...
				if(!empty($_POST['package_id'][$k])){
					...
					if($subscription_id=='' && $stb_id==''){
						...
						$stbArray = array(
							...
							'JOINING_DATE'           => $objMisc->changeDateFormat($_POST['installation_date'][$k]),
							'BOX_CHARGES'            => $_POST['box_charges'][$k],
						...
						$stbArray   = array(
							...
							'SUBSCRIPTION_ID'        => $subscription_id,
							'PACKAGE_ID'             => $_POST['package_id'][$k],
							'LA_CARTE_ID'            => (($_POST['la_carte_id'][$k]=='')?0:$_POST['la_carte_id'][$k]),
							'RELATION'               => (($_POST['relation'][$k]=='')?'':$_POST['relation'][$k]),
							'RELATIVE'               => (($_POST['relative'][$k]=='')?'':$_POST['relative'][$k]),
							'TYPE_ID'                => (($_POST['type_id'][$k]=='')?'':$_POST['type_id'][$k]),
							'ID_NO'                  => (($_POST['id_no'][$k]=='')?'':$_POST['id_no'][$k]),
							'INSTALLED_BY'           => (($_POST['installed_by'][$k]=='')?'':$_POST['installed_by'][$k]),
							'JOINING_DATE'           => (($_POST['installation_date'][$k]=='')?date('Y-m-d H:i:s'):$objMisc->changeDateFormat($_POST['installation_date'][$k])),
							'BOX_CHARGES'            => (($_POST['box_charges'][$k]=='')?0:$_POST['box_charges'][$k]),
							'IDENTITY_NAME'          => (($_POST['identity_name'][$k]=='')?'':$_POST['identity_name'][$k]),
							'LA_CARTE_NAME'          => (($_POST['la_carte_name'][$k]=='')?'':$_POST['la_carte_name'][$k]),
							'LA_CARTE_AMOUNT'        => (($_POST['la_carte_amount'][$k]=='')?0:$_POST['la_carte_amount'][$k]),
							'ADDED_BY'               => $_SESSION['USER_ID'],
							'STATUS'                 =>'A',
							'ADDED_TIME'             => date('Y-m-d H:i:s')
						);
						//print_r($stbArray);exit;
						$valstb = $objMisc->insert("stb_box",$stbArray);
						$stb_id = mysqli_insert_id($_SESSION['CONN']);
						$packageId  = $_POST['package_id'][$k];
						$PackageAmt = $objMisc->GiveValue("PACKAGE_ID='$packageId'",'PARENT_CHARGES','package_master');  
						$amount = $PackageAmt+$_POST['la_carte_amount'][$k];
						/*$rowArray       = array(
...
//echo "SELECT S.STB_NO,S.SUBSCRIPTION_ID,B.STATUS,B.`STB_ID` FROM `stb_box` B JOIN `subscriptions` S ON S.`SUBSCRIPTION_ID`=B.`SUBSCRIPTION_ID` WHERE B.SUBSCRIBER_ID='$rowRec[subscriber_id]' AND (B.`REMARKS`NOT LIKE 'Permanent Disconnection%' AND B.`REMARKS` NOT LIKE 'Box is Faulty.%')";
$sql=$objMisc->getAllRecordsNew("SELECT S.STB_NO,S.SUBSCRIPTION_ID,B.STATUS,B.`STB_ID` FROM `stb_box` B JOIN `subscriptions` S ON S.`SUBSCRIPTION_ID`=B.`SUBSCRIPTION_ID` WHERE B.SUBSCRIBER_ID='$rowRec[subscriber_id]' AND (B.`REMARKS`NOT LIKE 'Permanent Disconnection%' AND B.`REMARKS` NOT LIKE 'Box is Faulty.%' OR B.REMARKS IS NULL)");
...
<input type="text" name="installation_date[]" id="installation_date'.$i.'" class="form-control" value="">
</div>
<div class="form-group col-lg-2">
<label>Id Type </label>
...
<div class="form-group col-lg-2" >
<label>
<input type="radio" name="relation[]" id="relation'.$i.'" value="S/O"> S/o
</label>
<label>
...
<div class="form-group col-lg-2">
<label>Installation Date</label>
<input type="text" name="installation_date[]" id="installation_date'.$i.'" class="form-control" value="'.date('d/m/Y',strtotime($recs['joining_date'])).'">
</div>
<div class="form-group col-lg-2">
<label>Id Type </label>
...
<div class="form-group col-lg-2" >
<label>
	<input type="radio" name="relation[]" id="relation1" value="S/O"> S/o
</label>
<label>
...
$rowUtility=array('STATUS' => 'D');
}else if($reason=='Permanent Disconnection'){
$row1 = array(
'STATUS' => 'A',
'AVAILABILITY'=>'AV',
'REMARKS'=>$reason.' | Updated on '.$date,
...
$rowUtility=array('STATUS' => 'A');
}else if($reason=='Box Returned'){
$row1 = array(
'STATUS' => 'A',
'AVAILABILITY'=>'AV',
'REMARKS'=>$reason.' | Updated on '.$date,
...
$sub=$objMisc->GiveValue("SUBSCRIBER_ID='$id'",'NAME','subscribers');
$table.='<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header">
<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
<h4 class="modal-title" id="myModalLabel">'.$sub.' Ledger</h4>
</div>
...
$collection=$objMisc->getAllrecordsNew("SELECT `MONTH_DATE`,`ID`,RECEIPT_NO,MANUAL_RECEIPT_NO,AMOUNT_TYPE,AMOUNT,DISCOUNT FROM `monthly_charges` WHERE SUBSCRIBER_ID='$id' AND STATUS='A' ORDER BY `ID` DESC");
$balance=0;
$table.='<tr>
<th>Receipt No</th>
<th>Date</th>
...
$openingBala=$objMisc->GiveValues(" AMOUNT_TYPE='C' AND SUBSCRIBER_ID='$id' AND RECEIPT_NO='Opening Balance' AND STATUS='A'",'SUM(AMOUNT)as AMT,MONTH_DATE','monthly_charges');
if($openingBala['amt']>0){
$balance=$balance-$openingBala['amt'];
$table.='<tr>
<td>Opening Balance</td>
...
$monthDate='';*/
$bal=$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND AMOUNT_TYPE='D' AND STATUS='A'",'SUM(AMOUNT)','monthly_charges')-$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND AMOUNT_TYPE='C' AND STATUS='A'",'SUM(AMOUNT)','monthly_charges')-$objMisc->GiveValue("SUBSCRIBER_ID='$id' AND AMOUNT_TYPE='C' AND STATUS='A'",'SUM(DISCOUNT)','monthly_charges');
$i=1;
		...
		<div class="modal-content">
			...
			<div class="modal-body" id="modalData">';
				$table.= '<table style="width:100%" class="table  table-bordered table-striped">';
					...
					if($sub['subscriber_gst']!=''){
						$table.='<tr>
							<th colspan="2" width="13%">Subscriber GSTIN</th><td colspan="2">'.$sub['subscriber_gst'].'</td>
						</tr>';
					}
					
					$table.='<tr>
						<th>Name</th><td>'.$sub['name'].'</td>
						<th>D.O.B</th><td>'.(($sub['dob']!='0000-00-00')?$objMisc->DateFormat($sub['dob']):'N/A').'</td>
					</tr>';
					...
					$table.='<tr>
						<th>Joining Date</th><td>'.(($sub['connection_date']!='0000-00-00')?$objMisc->dateFormat($sub['connection_date']):'').'</td>
				...
				<table class="table table-bordered table-striped" style="font-size:11px;">';
					...
					$rec=$objMisc->getAllRecordsNew("SELECT B.STB_NO,B.VC_NO,B.MAC_NO,B.REMARKS,P.NAME,P.PARENT_CHARGES,S.* FROM `stb_box` S JOIN `subscriptions` B ON S.SUBSCRIPTION_ID=B.SUBSCRIPTION_ID JOIN `package_master` P ON P.PACKAGE_ID=S.PACKAGE_ID WHERE SUBSCRIBER_ID='$id'");
					$q=1;
					...
					foreach ($rec as $k => $value) {
						...
						if($value['status']=='A'){
							...
							<td>'.$q.'</td>
							<td>'.$value['stb_no'].'<br>VC No.'.$value['vc_no'].'<br>Mac No.'.$value['mac_no'].'</td>
							<td>'.$value['identity_name'].'<br>'.$value['relation'].' '.$value['relative'].(($value['type_id']!='')?'<br>ID type '.$value['type_id'].(($value['id_no']!='')?'<br>ID No. '.$value['id_no']:''):'').'</td>
							<td>'.$value['installed_by'].(($value['box_charges']!='')?'<br>Fees '.$value['box_charges']:'').(($value['joining_date']!='0000-00-00 00:00:00')?'<br>Date '.$objMisc->DateFormat($value['joining_date']):'').'</td>
							<td>'.$value['name'].'</td>
